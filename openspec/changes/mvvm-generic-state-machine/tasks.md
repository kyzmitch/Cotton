...
- [ ] 1.5 Update `BaseViewModel` to own/use the machine and publish state after successful `sendAction`
- [ ] 1.6 Align `ViewModelInterface` default `sendAction` implementations with the machine-backed path (no direct `state.transitionOn`)
...
- [ ] 2.3 Add BaseViewModel tests with an injectable/fake strategy for success and error paths
...
- [ ] 3.4 Migrate `SearchBarState` hierarchy (`InViewMode` / `InSearchMode`) into handler/strategy dispatch without `ViewModelState.transitionOn`
...
- [ ] 4.3 Confirm `StateMachineV2` / `ViewModelV2` remain unused by production adopters (no new dependencies on V2)
