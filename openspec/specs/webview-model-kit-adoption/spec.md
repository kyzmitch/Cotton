# webview-model-kit-adoption Specification

## Purpose
WebView adopts ViewModelKit `BaseViewModel` + `ViewModelStateMachine` with async `StateTransitioning`, a state context for loading commands and DoH/DNS/plugin/tab side effects, and consumer migration to `sendAction` + `statePublisher`.
