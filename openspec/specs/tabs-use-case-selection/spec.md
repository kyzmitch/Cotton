# tabs-use-case-selection Specification

## Purpose
Tab selection decisions after add and close move from `TabsDataService` into the split use cases (`AddTabUseCase` / `CloseTabUseCase`) driven by `TabSelectionStrategy`, while the existing separate use cases remain the write API for call sites.
