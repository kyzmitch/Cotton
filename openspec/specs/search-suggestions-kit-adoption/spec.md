# search-suggestions-kit-adoption Specification

## Purpose
SearchSuggestions adopts ViewModelKit `BaseViewModel` + `ViewModelStateMachine` with async `StateTransitioning`, a state context for known-domains and autocomplete side effects, sequenced `sendAction` so intermediate `.knownDomainsLoaded` still publishes, and consumer migration to the kit typealias + `statePublisher`.
