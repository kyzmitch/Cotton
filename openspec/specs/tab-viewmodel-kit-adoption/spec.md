# tab-viewmodel-kit-adoption Specification

## Purpose
Tab adopts ViewModelKit `BaseViewModel` + `ViewModelStateMachine` with async `StateTransitioning`, a state context for tab/favicon side effects, observer→action adapters, and consumer migration to `sendAction` + `statePublisher`.
